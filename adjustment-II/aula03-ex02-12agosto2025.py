...
# a) Identificar se os sistemas de equações são consistentes e determinados.
print("Como se pode calcular a matriz inversa e o determinante é zero, pode-se afirmar que o sistema é CONSISTENTE E INDETERMINADO")

# b) Encontrar a solução que melhor representa o sistema, utilizando MMQ
...
# a) Identificar se os sistemas de equações são consistentes e determinados.
"""
det(A_transposta * A) = [0.000]

Se o determinante for:
detA = 0 → INDETERMINADO
detA ≠ 0 → DETERMINADO
"""

# Para calcular det(Aᵀ * A), primeiro calcula a matriz transposta e o produto
ATA = A.T @ A

det = np.linalg.det(ATA)

# Vamos definir um limite para considerar o determinante como zero, devido à precisão numérica
limiar = 1e-10

if abs(det) < limiar:
    print("Determinante é zero → SISTEMA INDETERMINADO")
else:
    print("Determinante diferente de zero → SISTEMA DETERMINADO")
